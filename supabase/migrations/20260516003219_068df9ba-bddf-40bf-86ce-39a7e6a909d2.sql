...
BEGIN
  SELECT COUNT(*),
         ...
         COUNT(*) FILTER (WHERE period > now() - interval '30 days')
...
BEGIN
  FOR r IN SELECT DISTINCT provider_name FROM public.normalized_metrics WHERE provider_name IS NOT NULL LOOP
    PERFORM public.compute_source_iq_scorecard(r.provider_name);
    n := n + 1;
  END LOOP;
  RETURN n;
END;
