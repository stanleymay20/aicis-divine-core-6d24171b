-- 1. Metric push function (uses the schema Quantivis accepts)
...
BEGIN
  ...
  IF v_url IS NULL OR v_url = '' THEN
    SELECT decrypted_secret INTO v_url FROM vault.decrypted_secrets WHERE name = 'QUANTIVIS_WEBHOOK_URL' LIMIT 1;
  END IF;
  IF v_url IS NULL OR v_url = '' THEN
  ...
  WITH fresh AS (
    SELECT 
      COALESCE(country_iso3, 'WLD') AS country,
      to_char(observed_at::date, 'YYYY-MM-DD') AS date,
  ...
  SELECT jsonb_agg(jsonb_build_object(
         ...
         MAX(created_at),
         COUNT(*)
  INTO v_records, v_max_seen, v_count
  FROM fresh;
...
-- 2. Event push function for high-severity signals
...
BEGIN
  ...
  WITH fresh AS (
    SELECT
      COALESCE(country_iso3, 'WLD') AS country,
      to_char(occurred_at::date, 'YYYY-MM-DD') AS date,
  ...
  SELECT jsonb_agg(jsonb_build_object(
           'country', country, 'date', date, 'indicator', indicator, 'value', value
