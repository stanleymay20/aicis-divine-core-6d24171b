...
BEGIN
  ...
  -- Generate entity_links from co-occurring metrics (countries that share the same domain/metric patterns)
  WITH country_entities AS (
    SELECT ce.id, ce.iso3, ce.canonical_name
  ...
  -- Find pairs of countries with metrics in the same domain
  domain_pairs AS (
    SELECT DISTINCT
      ce1.id AS source_id,
      ce2.id AS target_id,
      nm1.domain AS shared_domain
    ...
    WHERE ce1.id < ce2.id  -- avoid duplicates
    LIMIT 5000
  ),
  ins AS (
    INSERT INTO entity_links (source_entity_id, target_entity_id, link_type, strength, metadata)
    SELECT 
      dp.source_id, dp.target_id, 'domain_co_occurrence',
      0.6,
      jsonb_build_object('shared_domain', dp.shared_domain, 'generated_by', 'batch_generate_entity_links')
