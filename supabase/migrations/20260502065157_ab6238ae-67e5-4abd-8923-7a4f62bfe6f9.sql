CREATE TABLE IF NOT EXISTS public.aicis_early_warnings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  warning_kind TEXT NOT NULL CHECK (warning_kind IN ('rising_cluster','repeated_locality','severity_spike','cross_border_spillover','weak_signal')),
  iso3 TEXT,
  locality TEXT,
  admin_level_1 TEXT,
  ...
  escalation_probability NUMERIC NOT NULL DEFAULT 0 CHECK (escalation_probability BETWEEN 0 AND 1),
...
CREATE INDEX IF NOT EXISTS idx_warnings_kind ON public.aicis_early_warnings (warning_kind, status);
CREATE INDEX IF NOT EXISTS idx_warnings_iso3 ON public.aicis_early_warnings (iso3, locality);
CREATE INDEX IF NOT EXISTS idx_warnings_recent ON public.aicis_early_warnings (first_detected_at DESC);

CREATE TABLE IF NOT EXISTS public.aicis_warning_evidence (
  warning_id UUID NOT NULL REFERENCES public.aicis_early_warnings(id) ON DELETE CASCADE,
  local_event_id UUID NOT NULL REFERENCES public.aicis_local_events(id) ON DELETE CASCADE,
  contribution NUMERIC DEFAULT 1.0,
  PRIMARY KEY (warning_id, local_event_id)
);
CREATE INDEX IF NOT EXISTS idx_warning_evidence_warning ON public.aicis_warning_evidence (warning_id);

CREATE TABLE IF NOT EXISTS public.aicis_warning_snapshots (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  snapshot_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  total_warnings INTEGER NOT NULL DEFAULT 0,
  by_kind JSONB DEFAULT '{}'::jsonb,
  by_country JSONB DEFAULT '{}'::jsonb,
  avg_escalation NUMERIC,
  avg_confidence NUMERIC
);

ALTER TABLE public.aicis_early_warnings ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.aicis_warning_evidence ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.aicis_warning_snapshots ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "warnings_read" ON public.aicis_early_warnings;
CREATE POLICY "warnings_read" ON public.aicis_early_warnings FOR SELECT TO authenticated
  USING (public.has_role(auth.uid(),'admin'::app_role) OR public.has_role(auth.uid(),'operator'::app_role) OR public.has_role(auth.uid(),'observer'::app_role));

DROP POLICY IF EXISTS "warning_evidence_read" ON public.aicis_warning_evidence;
CREATE POLICY "warning_evidence_read" ON public.aicis_warning_evidence FOR SELECT TO authenticated
  USING (public.has_role(auth.uid(),'admin'::app_role) OR public.has_role(auth.uid(),'operator'::app_role) OR public.has_role(auth.uid(),'observer'::app_role));

DROP POLICY IF EXISTS "warning_snapshots_read" ON public.aicis_warning_snapshots;
CREATE POLICY "warning_snapshots_read" ON public.aicis_warning_snapshots FOR SELECT TO authenticated
  USING (public.has_role(auth.uid(),'admin'::app_role) OR public.has_role(auth.uid(),'operator'::app_role) OR public.has_role(auth.uid(),'observer'::app_role));
...
BEGIN
  ...
  -- D. CROSS-BORDER SPILLOVER (haversine adjacency <2000km)
  FOR r IN
    WITH base AS (
    ...
    pairs AS (
      ...
      WHERE EXISTS (
        SELECT 1 FROM (SELECT iso3, AVG(lat) lat, AVG(lon) lon FROM aicis_geo_entities WHERE lat IS NOT NULL GROUP BY iso3) ga
        JOIN (SELECT iso3, AVG(lat) lat, AVG(lon) lon FROM aicis_geo_entities WHERE lat IS NOT NULL GROUP BY iso3) gb ON true
        WHERE ga.iso3=a.iso3 AND gb.iso3=b.iso3
          AND (6371*acos(LEAST(1,cos(radians(ga.lat))*cos(radians(gb.lat))*cos(radians(gb.lon-ga.lon))+sin(radians(ga.lat))*sin(radians(gb.lat))))) < 2000
      )
    )
    SELECT * FROM pairs LIMIT 200
  LOOP
    ...
    VALUES ('cross_border_spillover', r.iso_a||','||r.iso_b, r.event_type, r.subtype, r.sev, r.conf,
      ...
      jsonb_build_object('iso_a',r.iso_a,'iso_b',r.iso_b),
