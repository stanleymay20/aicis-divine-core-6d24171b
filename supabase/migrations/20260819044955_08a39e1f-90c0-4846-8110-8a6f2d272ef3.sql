CREATE OR REPLACE FUNCTION public.compute_pns_certification_fix_gates()
RETURNS void LANGUAGE sql AS $$ SELECT 1 $$;
DROP FUNCTION public.compute_pns_certification_fix_gates();
...
BEGIN
  ...
  g := g || jsonb_build_object('gate','G_causal','metric','causal propagation events (7d)','target',50,'value',n,
       'status', CASE WHEN n >= 50 THEN 'PASS' ELSE 'FAIL' END,'evidence','planetary_propagation_events.generated_at count over 7d');
  ...
  g := g || jsonb_build_object('gate','J_governance','metric','dual-approval actions executed without two approvers','target',0,'value',n,
       'status', CASE WHEN n = 0 THEN 'PASS' ELSE 'FAIL' END,'evidence','risk_action_recommendations requiring dual approval executed with a missing approver');
