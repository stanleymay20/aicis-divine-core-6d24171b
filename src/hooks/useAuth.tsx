...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const handleAuthChange = (event: AuthChangeEvent, nextSession: Session | null) => {
      // INITIAL_SESSION is storage-derived and must not unlock protected UI until
      // the token is verified against the auth server. Fresh sign-in/refresh and
      // recovery events are authoritative responses from Supabase Auth.
    ...
    // Some auth client versions can delay INITIAL_SESSION, so explicitly read the
    // stored candidate as a bounded fallback. It is still server-verified before use.
  ...
  const signOut = useCallback(async () => {
    try {
      ...
      // Local credentials must be removed even when the network is unavailable.
      await supabase.auth.signOut({ scope: "local" });
    } finally {
      setSession(null);
      setUser(null);
