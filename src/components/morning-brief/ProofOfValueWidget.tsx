    ...
    queryFn: async () => {
      const [decisionsRes, outcomesRes, measuredRes] = await Promise.all([
        supabase
          ...
          .select("id, action_taken, outcome_success, evidence_type, roi_estimate, net_value, execution_status")
        ...
        supabase
          .from("decision_outcome_log")
          .select("id, roi_estimate, net_value")
      ...
      const totalROI = outcomes.reduce((sum, o) => sum + (Number(o.roi_estimate) || 0), 0);
      const avgROI = outcomes.length > 0 ? totalROI / outcomes.length : 0;
      const totalNetValue = outcomes.reduce((sum, o) => sum + (Number(o.net_value) || 0), 0);
      ...
      <CardContent>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <ValueStat
            ...
            label="Net Value"
            value={data.totalNetValue > 0 ? formatCurrency(data.totalNetValue) : "—"}
            sub={data.totalNetValue > 0 ? "total captured" : "Not yet recorded"}
          />
          <ValueStat
            icon={<DollarSign className="h-3.5 w-3.5 text-primary" />}
            label="Avg ROI"
            value={data.avgROI > 0 ? `${data.avgROI.toFixed(1)}x` : "—"}
            sub={data.avgROI > 0 ? "per decision" : "Not yet recorded"}
          />
        </div>
      </CardContent>
    </Card>
