...
import {
  Search, X, Globe, MapPin, TrendingUp, TrendingDown, Minus, ArrowRight,
  AlertTriangle, Eye, Bookmark, FileText, RotateCcw, Layers, Filter,
} from "lucide-react";
import L from "leaflet";
import "leaflet/dist/leaflet.css";
import { parseMapQuery, MapQuery, ALL_DOMAINS } from "./queryParser";
import { useCountryRiskData, aggregateByCountry, useRelatedSignals, CountryRiskRow } from "./useAtlasData";
...
export function RiskAtlas() {
  ...
  // Compute map center/zoom based on query
  ...
  return (
    <div className="h-[calc(100vh-140px)] flex flex-col">
      {/* Header */}
      <div className="flex items-center gap-2 px-4 py-3 border-b border-border bg-background/95 backdrop-blur">
      ...
      {activeQuery && (
        <div className="px-4 py-1.5 bg-primary/5 border-b border-primary/20 flex items-center gap-2 text-xs">
...
/* ════════════════════════════════════════════
   AtlasMap – Leaflet map with dynamic markers
   ════════════════════════════════════════════ */
...
function AtlasMap({ countries, center, zoom, selectedCountry, onCountryClick, query }: AtlasMapProps) {
  ...
  useEffect(() => {
    ...
    for (const c of Object.values(countries)) {
      ...
      // Direction indicator
      const dirIcon = c.direction === "up" ? "↑" : c.direction === "down" ? "↓" : "";
      ...
      marker.bindPopup(`
        <div style="min-width:180px;font-family:system-ui">
          ...
          <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
            ...
            ${dirIcon ? `<span style="font-size:16px">${dirIcon}</span>` : ""}
          </div>
          <div style="color:#999;font-size:12px">${c.domains} domains tracked</div>
        </div>
      `);
