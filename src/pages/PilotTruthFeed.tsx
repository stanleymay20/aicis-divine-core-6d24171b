...
import { AlertTriangle, CheckCircle2, Clock, Lock, TrendingUp } from "lucide-react";
...
export default function PilotTruthFeed() {
  ...
  const summary = useQuery({
    ...
    queryFn: async () => {
      const [outcomes, locked, expired, proposed] = await Promise.all([
        ...
        supabase.from("risk_action_recommendations").select("*", { count: "exact", head: true }).eq("status", "expired"),
        supabase.from("risk_action_recommendations").select("*", { count: "exact", head: true }).eq("status", "proposed"),
      ]);
      return {
    ...
    <div className="container max-w-7xl mx-auto p-4 md:p-8 space-y-6">
      <header className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground max-w-3xl">
          Externally defensible record: recorded decision outcomes joined with executed actions and locked realized
          forecasts. Locked forecasts cannot be retroactively edited.
        </p>
      </header>

      {/* Summary tiles */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <SummaryTile icon={<CheckCircle2 className="h-4 w-4" />} label="Outcomes Logged" value={summary.data?.outcomes ?? "…"} />
        <SummaryTile icon={<Lock className="h-4 w-4" />} label="Locked Forecasts" value={summary.data?.lockedForecasts ?? "…"} />
        <SummaryTile icon={<Clock className="h-4 w-4" />} label="Proposed Actions" value={summary.data?.proposed ?? "…"} tone="warn" />
        <SummaryTile icon={<AlertTriangle className="h-4 w-4" />} label="Auto-Expired" value={summary.data?.expired ?? "…"} tone="muted" />
      </div>

      {/* Stale actions nag panel */}
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base">
            <Clock className="h-4 w-4 text-amber-500" />
            Stale Proposed Actions ({stale.data?.length ?? 0})
          </CardTitle>
          <p className="text-xs text-muted-foreground">
            Sorted oldest first. Auto-expiry sweep runs every 30 minutes.
          </p>
        </CardHeader>
        <CardContent className="space-y-2">
          ...
          {stale.data?.length === 0 && (
            <div className="text-sm text-muted-foreground py-6 text-center">
              ✓ No stale proposed actions.
            </div>
          )}
          {stale.data?.slice(0, 12).map((a) => {
            const ageHours = (Date.now() - new Date(a.created_at).getTime()) / 3.6e6;
            const overdue = ageHours > a.urgency_hours;
            return (
              <div
                key={a.id}
                className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 p-3 rounded-lg border bg-card/50"
              >
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          ...
          {truth.data?.slice(0, 30).map((r) => (
            <div key={r.outcome_id} className="p-3 rounded-lg border bg-card/50 space-y-1">
              <div className="flex items-center gap-2 flex-wrap">
