...
type KpiRow = {
  ...
  active_strategic_events: number | null;
  forecast_escalation_index: number | null;
  institutional_trust_score: number | null;
  evidence_quality_score: number | null;
  top_risk_domain: string | null;
  top_risk_region: string | null;
...
export default function PlanetaryCommandCenter() {
  ...
  const insights = useQuery({
    ...
    queryFn: () => viewQuery<InsightRow>("executive_planetary_insights_view", 8),
    refetchInterval: 60_000,
  });

  const telemetry = useQuery({
    queryKey: ["planetary-command-telemetry"],
    queryFn: () => viewQuery<SimpleCommandRow>("telemetry_backbone_command_view", 8),
    refetchInterval: 45_000,
  });

  const agents = useQuery({
    queryKey: ["planetary-command-agents"],
    queryFn: () => viewQuery<SimpleCommandRow>("agent_task_command_view", 8),
    refetchInterval: 60_000,
  });

  const causal = useQuery({
    queryKey: ["planetary-command-causal"],
    queryFn: () => viewQuery<SimpleCommandRow>("planetary_causal_command_view", 8),
    refetchInterval: 60_000,
  });

  const interventions = useQuery({
    queryKey: ["planetary-command-interventions"],
    queryFn: () => viewQuery<SimpleCommandRow>("intervention_governance_command_view", 8),
    refetchInterval: 60_000,
  });

  const memory = useQuery({
    queryKey: ["planetary-command-memory"],
    queryFn: () => viewQuery<SimpleCommandRow>("memory_forecast_command_view", 8),
    refetchInterval: 120_000,
  });

  const briefings = useQuery({
    queryKey: ["planetary-command-briefings"],
    queryFn: () => viewQuery<SimpleCommandRow>("executive_briefing_command_view", 5),
    refetchInterval: 120_000,
  });

  const translations = useQuery({
    queryKey: ["planetary-command-translations"],
    queryFn: () => viewQuery<SimpleCommandRow>("translation_operations_command_view", 6),
    refetchInterval: 120_000,
  });
  ...
  const summary = useMemo(() => {
    const risk = formatScore(kpi?.global_risk_index);
    const readinessScore = formatScore(ready?.overall_enterprise_score);
    const coverage = formatScore(kpi?.telemetry_coverage_score ?? ready?.telemetry_coverage_score);
    return { risk, readinessScore, coverage };
  }, [kpi, ready]);
  ...
  return (
    <AICISLayout>
      <div className="p-4 md:p-6 lg:p-8 max-w-[1500px] mx-auto h-full overflow-y-auto space-y-5 animate-fade-in">
        <div className="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-2 max-w-3xl">
              Unified operational view for executive KPIs, telemetry backbone, causal propagation, intervention governance, memory, briefings, translation, and multi-agent coordination.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
          <Card className="xl:col-span-2 border-border bg-card/70">
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2"><Sparkles className="h-4 w-4 text-primary" /> Executive Intelligence</CardTitle>
              <CardDescription>Current boardroom-ready judgments generated from the backend intelligence layer.</CardDescription>
            </CardHeader>
            <CardContent>
              <CommandList
                loading={insights.isLoading}
                empty="No executive insights generated yet. Run run_executive_dashboard_cycle() after applying migrations."
                rows={(insights.data ?? []).map((r) => ({
                  title: r.insight_title ?? "Untitled insight",
                  meta: r.severity_band ?? "monitor",
                  summary: r.insight_summary ?? "No summary available.",
                  badgeTone: severityTone(r.severity_band),
                  footer: r.recommended_action,
                }))}
              />
            </CardContent>
          </Card>
          ...
          <Card className="border-border bg-card/70">
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-primary" /> Readiness</CardTitle>
              <CardDescription>Commercial enterprise posture.</CardDescription>
            </CardHeader>
              ...
              {readiness.isLoading ? <Skeleton className="h-48 w-full" /> : ready ? (
                <>
                  <div className="flex items-center justify-between rounded-lg border border-border p-3 bg-background/40">
                    ...
                    <Badge variant="outline" className={toneForScore(ready.overall_enterprise_score)}>{ready.readiness_grade ?? "unknown"}</Badge>
                  </div>
                  ...
                  <ScoreLine label="Incident Response" value={ready.incident_response_score} />
                </>
              ) : (
        ...
        <Card className="border-border bg-card/70">
          <CardHeader className="pb-3">
            <CardTitle className="text-base flex items-center gap-2"><Languages className="h-4 w-4 text-primary" /> Translation Operations</CardTitle>
            <CardDescription>Language localization queue so global intelligence becomes useful to each user.</CardDescription>
          </CardHeader>
          <CardContent>
            <CommandList
              loading={translations.isLoading}
              empty="No translation queue activity yet. Run run_translation_intelligence_cycle()."
              rows={(translations.data ?? []).map((r) => ({
                title: `${r.target_language_code ?? "language"} · ${r.queue_status ?? "status"}`,
                meta: `${r.item_count ?? 0} items`,
                summary: `Average priority ${r.avg_priority ?? 0}`,
                badgeTone: severityTone(r.queue_status === "queued" ? "strategic" : r.queue_status === "completed" ? "healthy" : "monitor"),
              }))}
            />
          </CardContent>
        </Card>
...
function MetricCard({ icon, label, value, suffix, tone, loading }: { icon: React.ReactNode; label: string; value: number; suffix: string; tone: string; loading?: boolean }) {
  return (
    <Card className="border-border bg-card/70">
      <CardContent className="p-4">
        <div className="flex items-center justify-between gap-2">
          ...
          <Badge variant="outline" className={tone}>Live</Badge>
        </div>
        {loading ? <Skeleton className="h-8 w-24 mt-3" /> : <div className="text-3xl font-semibold mt-3">{value}<span className="text-base text-muted-foreground">{suffix}</span></div>}
      </CardContent>
    </Card>
  );
}
...
function ScoreLine({ label, value }: { label: string; value?: number | null }) {
  const v = formatScore(value);
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-xs"><span className="text-muted-foreground">{label}</span><span>{v}/100</span></div>
      <div className="h-2 rounded-full bg-muted overflow-hidden"><div className="h-full bg-primary" style={{ width: `${Math.max(0, Math.min(100, v))}%` }} /></div>
    </div>
  );
}

function SurfaceCard({ icon, title, loading, empty, rows }: { icon: React.ReactNode; title: string; loading: boolean; empty: string; rows: Array<{ title: string; meta?: string; summary?: string; badgeTone?: string }> }) {
  return (
    <Card className="border-border bg-card/70 min-h-[320px]">
      <CardHeader className="pb-3">
        <CardTitle className="text-base flex items-center gap-2">{icon}{title}</CardTitle>
      </CardHeader>
      <CardContent>
        <CommandList loading={loading} empty={empty} rows={rows} compact />
      </CardContent>
    </Card>
  );
}

function CommandList({ loading, empty, rows, compact = false }: { loading: boolean; empty: string; compact?: boolean; rows: Array<{ title: string; meta?: string; summary?: string; badgeTone?: string; footer?: string | null }> }) {
  if (loading) return <div className="space-y-2"><Skeleton className="h-16 w-full" /><Skeleton className="h-16 w-full" /><Skeleton className="h-16 w-full" /></div>;
  if (!rows.length) return <EmptyState text={empty} />;
  return (
    <ScrollArea className={compact ? "h-[230px] pr-2" : "h-[310px] pr-2"}>
      <div className="space-y-2">
        {rows.map((row, idx) => (
          <div key={`${row.title}-${idx}`} className="rounded-lg border border-border bg-background/40 p-3">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="text-sm font-medium truncate">{row.title}</p>
                {row.summary && <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{row.summary}</p>}
                {row.footer && <p className="text-xs text-primary mt-2 line-clamp-1">{row.footer}</p>}
              </div>
              {row.meta && <Badge variant="outline" className={`shrink-0 text-[10px] ${row.badgeTone ?? ""}`}>{row.meta}</Badge>}
            </div>
          </div>
        ))}
      </div>
...
function EmptyState({ text }: { text: string }) {
  return (
    <div className="rounded-lg border border-dashed border-border bg-background/30 p-4 text-sm text-muted-foreground flex items-start gap-2">
